...
void ArcBallCamera::ProcessMouseInput(float definedXPos, float definedYPos) {
    int mouseKeyState = glfwGetMouseButton(Window::GetGLFWwindowRef(), GLFW_MOUSE_BUTTON_LEFT);
...
float ArcBallCamera::GetFov() const {
    return m_fov;
}

float ArcBallCamera::GetNear() const {
    return m_near;
}

float ArcBallCamera::GetFar() const {
    return m_far;
}
...
void ArcBallCamera::RecreateProjectionMatrix(const glm::vec2 viewportSize) {
    if (m_isOrtho) {
        ...
        m_projectionMatrix
            = glm::perspective(glm::radians(m_fov), viewportSize.x / viewportSize.y, m_near, m_far);
    }
}
...
const glm::mat4 ArcBallCamera::GetProjecionMatrix(const glm::vec2 viewportSize) {
