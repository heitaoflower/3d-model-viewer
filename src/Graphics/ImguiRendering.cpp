...
void ImguiRendering::InitImgui(GLFWwindow *window)
{
    ...
    const std::string fontPath =
        std::string(GLOBAL_PATH) + std::string("res/font.ttf");
    ...
    builder.AddRanges(
        io.Fonts->GetGlyphRangesDefault()); // Přidáme jednu z výchozích sad
    builder.BuildRanges(&ranges); // Sestavíme výsledný výsledek (seřazené sady
                                  // se všemi jedinečnými znaky)

    auto font = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 20, nullptr,
                                             ranges.Data);
