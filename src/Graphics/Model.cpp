...
void Model::ProcessNode(aiNode *node, const aiScene *scene)
{
    ...
    {
        aiMesh *mesh =
            scene->mMeshes[node->mMeshes[i]]; // získáni meshe ze scény
        m_meshes.push_back(
                        ...
                        const aiScene *scene) // Z meshů získat vertexy
{
    ...
    if (!m_manualySetTextures)
    {
        ...
        {
            aiMaterial *material = scene->mMaterials[mesh->mMaterialIndex];
            std::vector<MeshData::Texture> diffuseMaps = LoadMaterialTextures(
            ...
            std::vector<MeshData::Texture> specularMaps = LoadMaterialTextures(
